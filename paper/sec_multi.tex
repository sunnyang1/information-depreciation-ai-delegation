...
The bound $\bar{\eta} < 1$ is not an exogenous assumption; it reflects fundamental limits of LLM information processing that no amount of attention can overcome. We provide two complementary foundations.

\textbf{Information-theoretic foundation.} In Appendix~\ref{app:rate_distortion}, we derive $\bar{\eta} < 1$ from the rate-distortion function for parallel Gaussian sources. The correct reverse water-filling solution $D(R) = \sum_k \min\{\lambda_k, \theta(R)\}$ implies that for any finite rate budget $R$, the distortion $D(R) > 0$, hence the preservation rate $\eta(R) = 1 - D(R)/\text{tr}(\Sigma) < 1$. This bound is tightest when the eigenvalue spectrum of the hidden-state covariance decays slowly.

\textbf{Architectural foundation.} Three structural features of transformers enforce the bound:
\begin{enumerate}[label=(\roman*)]
...
\subsection{The Scope of the Preservation Bound}
\label{subsec:preservation_scope}

Before characterizing the optimal depth, we address an important question raised by the reviewer: when can the preservation bound $\bar{\eta} < 1$ be circumvented by alternative architectures such as evidence buffers, raw-payload forwarding, or retrieval-augmented generation? We show that while these mechanisms raise the effective preservation rate, they cannot eliminate the bound.

\begin{proposition}[Memory and the Preservation Bound]\label{prop:memory_bound}
Suppose layer $\ell$ has access to an external memory of capacity $M \geq 0$ that can store raw signals from upstream layers. The effective preservation bound with memory is
\begin{equation}\label{eq:memory_eta}
\bar{\eta}_{\text{eff}}(M) = 1 - (1 - \bar{\eta}) \cdot e^{-\lambda M},
\end{equation}
where $\lambda > 0$ is the memory effectiveness parameter. Then:
\begin{enumerate}[label=(\roman*)]
\item $\bar{\eta}_{\text{eff}}(M)$ is strictly increasing and concave in $M$.
\item $\lim_{M \to \infty} \bar{\eta}_{\text{eff}}(M) = 1$, but for any finite $M$, $\bar{\eta}_{\text{eff}}(M) < 1$.
\item The finite-optimal-depth result (Proposition~\ref{prop:optimal_depth}) holds for any finite $M$.
\end{enumerate}
\end{proposition}

\begin{proof}
\textbf{Part (i):} Differentiating \eqref{eq:memory_eta}: $\partial \bar{\eta}_{\text{eff}} / \partial M = \lambda (1 - \bar{\eta}) e^{-\lambda M} > 0$ and $\partial^2 \bar{\eta}_{\text{eff}} / \partial M^2 = -\lambda^2 (1 - \bar{\eta}) e^{-\lambda M} < 0$.

\textbf{Part (ii):} As $M \to \infty$, $e^{-\lambda M} \to 0$, so $\bar{\eta}_{\text{eff}}(M) \to 1$. For any finite $M$, $e^{-\lambda M} > 0$, so $\bar{\eta}_{\text{eff}}(M) = 1 - \text{[positive]} < 1$.

\textbf{Part (iii):} Even with memory, the effective preservation rate at layer $\ell$ is $\eta^{\text{eff}}_\ell \leq \bar{\eta}_{\text{eff}}(M) < 1$. The signal contribution at layer $\ell$ is bounded by $C \cdot (\bar{\eta}_{\text{eff}})^{\ell}$, which decays geometrically. The proof of Proposition~\ref{prop:optimal_depth} applies unchanged.
\end{proof}

\paragraph{Implications for alternative architectures.} Proposition~\ref{prop:memory_bound} formalizes how alternative architectures affect the preservation bound. Table~\ref{tab:architecture_mapping} maps common ML primitives to changes in $(\bar{\eta}, A)$.

\begin{table}[htbp]
\centering
\small
\caption{Mapping from ML Primitives to Model Parameters}
\label{tab:architecture_mapping}
...
\toprule
\textbf{Architecture} & \textbf{Effect on $\bar{\eta}$} & \textbf{Effect on $A$} & \textbf{Source} \\
\midrule
RAG / Evidence buffers & $+0.05$ to $+0.15$ & Unchanged & \cite{lewis2020retrieval} \\
KV-cache memory & $+0.02$ to $+0.08$ & Unchanged & \cite{pope2023efficiently} \\
Skip connections & $+0.03$ to $+0.10$ & Unchanged & ResNet-style \\
Parallel branches & Unchanged & $+M\times$ (distributed) & Tree aggregation \\
FlashAttention & Unchanged & $+20$--$50\%$ effective & \cite{dao2022flashattention} \\
Larger embeddings & $+0.01$ to $+0.05$ & Unchanged & Dimensionality \\
\bottomrule
\end{tabular}
\end{table}

The key insight is that all of these architectures raise $\bar{\eta}$ but preserve the bound $\bar{\eta}_{\text{eff}} < 1$ for realistic memory capacities. Only with infinite memory (or perfect raw-signal forwarding with zero retrieval cost) would the bound vanish---a limit that is technologically unattainable.
