...
def run_experiment_6_cost_irrelevance(
    ...
    for kappa in kappa_values:
        ...
        print(f"  kappa = {kappa:7.4f}: L* = {L_star}, NetValue[L*] = {net_values[L_star-1]:.4f}")
...
def export_advanced_to_latex(
    ...
    output = []
    output.append("\n" + "=" * 70)
    output.append("ADVANCED EXPERIMENTS: LATEX TABLES")
    output.append("=" * 70)

    # Table 4: Signal Overload
    output.append("\n--- Table: Signal Overload ---")
    ...
    output.append("\n--- Table: Heterogeneity Reduces Distortion ---")
    output.append("\\begin{table}[htbp]")
    output.append("\\centering")
