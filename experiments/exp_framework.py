...
class TheoreticalLLMChain:
    ...
    def run_chain(
        ...
        # Precision retained = final posterior / initial prior
        initial_prior = float(n_signals * DEFAULT_TAU0) if n_signals > 0 else 1.0
        precision_retained = (
            final_state.tau_post / initial_prior if final_state and initial_prior > 0 else 0.0
        )
...
def export_results_to_latex_tables(
    ...
    output = []
    output.append("\n" + "=" * 70)
    output.append("LATEX TABLE OUTPUT")
    output.append("=" * 70)

    # Table 1: Depth-Accuracy
    output.append("\n--- Table: Depth-Accuracy Tradeoff ---")
    ...
    output.append("\n--- Table: Front-Loading Validation ---")
    output.append("\\begin{table}[htbp]")
    output.append("\\centering")
    ...
    output.append("\n--- Table: Information Depreciation Estimation ---")
    output.append("\\begin{table}[htbp]")
    output.append("\\centering")
